...
def main():
    ...
    scores_columns = ["sample_id","total_variants","contact_type_variants", "region_variants", "region_variants", "ACE2_contact_variants", "trimer_contact_variants", "barns_class_variants", "bloom_ACE2" , "VDS", "serum_escape", "mAb_escape_all_classes", "class_1_mAb_escape", "class_2_mAb_escape", "class_3_mAb_escape", "class_4_mAb_escape", "BEC_sample_EF", "BEC_REF"]
    ...
    for vcf in args.vcfs:
        ...
        if len(vcf) != 0: #do not add summary if the vcf file is empty (but the empty file has to be created so need to handle). 
            ...
            bec_ref_score = per_sample_output["BEC_RES"].str.split(",").explode().replace(r'^\s*$', np.nan, regex=True).astype("float").sum()
            scores_list = [sample_name,sample_variant_number, type_string, region_string, domain_string, ace2_contacts_score, trimer_contacts_score, barns_string,bloom_ace2_sum , vds_sum, serum_escape_sum, mab_escape_all_sum, mab_escape_class_1_sum, mab_escape_class_2_sum, mab_escape_class_3_sum, mab_escape_class_4_sum, bec_sample_ef_score, bec_ref_score]
            ...
            Path(f'{args.output_dir}/per_sample_annotation/{sample_name}.summary.tsv').touch() #touch file if empty  
