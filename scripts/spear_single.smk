...
rule summarise_vcfs:
   ...
   shell:
      """for i in {config[output_dir]}/final_vcfs/*.spear.vcf ; do grep -v '^#' $i || true | sed "s|^NC_045512.2|$i|g"; done > {config[output_dir]}/intermediate_output/anno_concat.tsv ; convert_format.py --is_vcf_input {config[vcf]} --is_filtered {config[filter]} {config[output_dir]}/intermediate_output/anno_concat.tsv {config[output_dir]} {config[data_dir]} {input} {config[samples]} 2> {log}"""
